...
export type UpdateEnkiiCommentParams = {
  owner: string;
  repo: string;
  commentId: number;
  body: string;
...
export type UpdateEnkiiCommentResult = {
  id: number;
  html_url: string;
  updated_at: string;
};
...
/**
 * Updates a enkii comment on GitHub (either an issue/PR comment or a PR review comment)
 *
 ...
 */
export async function updateEnkiiComment(
  octokit: Octokit,
  params: UpdateEnkiiCommentParams,
): Promise<UpdateEnkiiCommentResult> {
