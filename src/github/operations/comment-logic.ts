...
export function updateCommentBody(input: CommentUpdateInput): string {
  ...
  // First, remove the "enkii is working…" message (and support legacy wording)
  const workingPattern = /enkii is working[…\.]{1,3}(?:\s*<img[^>]*>)?/i;
  ...
  if (actionFailed) {
    header = "**enkii encountered an error";
    if (durationStr) {
    ...
    header = `**enkii finished @${username}'s task`;
    if (durationStr) {
